...
class Okhttp() {
    ...
    val okhttpClient = OkHttpClient.Builder()
        ...
        .connectionPool(ConnectionPool(1, 10, TimeUnit.MINUTES))
    ...
    fun registration(userDTO: UserDTO) {
        ...
        val request = Request.Builder().url(BASE_URL + "/registration").post(body).build()
        try {
            okhttpClient.newCall(request).enqueue(responseCallback = object : Callback {
                ...
                override fun onResponse(call: Call, response: Response) {
                    UserIdViewModel.loadUserId(response.body!!.string())
                    //Log.d(TAG, "registration answer ${response.body!!.string()}")
        ...
        CoroutineScope(Dispatchers.IO).launch {
            UserIdViewModel.stateUserId.collect {
                val request = Request.Builder()
                    .url(BASE_URL + "/open-sse-stream/${it}")
